<?php
/**
 * BirthChartController — REST endpoint alias for natal chart with full data.
 *
...
/**
 * Handles POST /astrologer/v1/birth-chart.
 *
 * Alias for the natal chart endpoint that returns the full data response
 * including SVG, positions, houses, aspects, and distributions.
 */
final class BirthChartController extends AbstractController {
	...
	/**
	 * Register the birth chart route.
	 */
	...
	/**
	 * Handle the birth chart request.
	 *
	...
	public function handle( WP_REST_Request $request ): WP_REST_Response {
		...
		// Always request SVG and full data for the birth-chart alias.
		$result = $this->chart_service->birthChart( $dto );
	...
	private function get_args(): array {
		$subject = SubjectSchema::get();
		$options = ChartOptionsSchema::get();

		$nested = array();
		foreach ( $subject as $key => $schema ) {
			$nested[ 'subject[' . $key . ']' ] = $schema;
		}
		...
		return array_merge( $extra, $nested, $options );
	}
}
