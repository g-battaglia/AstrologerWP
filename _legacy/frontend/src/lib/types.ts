...
export function validateSubjectForm(data: SubjectFormData): SubjectFormErrors {
    ...
    if (!data.day.trim() || isNaN(day) || day < 1 || day > 31) {
        errors.day = t('errorDay', 'Day must be between 1 and 31');
    }

    // Month: required, integer 1–12
    const month = parseInt(data.month, 10);
    if (!data.month.trim() || isNaN(month) || month < 1 || month > 12) {
        errors.month = t('errorMonth', 'Month must be between 1 and 12');
    }

    // Year: required, integer 1–2200
    const year = parseInt(data.year, 10);
    if (!data.year.trim() || isNaN(year) || year < 1 || year > 2200) {
        errors.year = t('errorYear', 'Year must be between 1 and 2200');
    }
    ...
    if (!errors.day && !errors.month && !errors.year) {
        ...
        if (day > maxDay) {
            errors.day = t('errorDayMonth', `Day must be between 1 and ${maxDay} for this month`);
        }
    }
    ...
    if (!data.hour.trim() || isNaN(hour) || hour < 0 || hour > 23) {
        errors.hour = t('errorHour', 'Hour must be between 0 and 23');
    }

    // Minute: required, integer 0–59
    const minute = parseInt(data.minute, 10);
    if (!data.minute.trim() || isNaN(minute) || minute < 0 || minute > 59) {
        errors.minute = t('errorMinute', 'Minutes must be between 0 and 59');
    }

    // Latitude: required, number -90 to 90
    const lat = parseFloat(data.latitude);
    if (!data.latitude.trim() || isNaN(lat) || lat < -90 || lat > 90) {
        errors.latitude = t('errorLatitude', 'Latitude must be between -90 and 90');
    }

    // Longitude: required, number -180 to 180
    const lng = parseFloat(data.longitude);
    if (!data.longitude.trim() || isNaN(lng) || lng < -180 || lng > 180) {
        errors.longitude = t('errorLongitude', 'Longitude must be between -180 and 180');
    }

    // City: required
    if (!data.city.trim()) {
        errors.city = t('errorCity', 'City is required');
    }

    // Nation: required
    if (!data.nation.trim()) {
        errors.nation = t('errorNation', 'Country code is required');
    }

    // Timezone: required
    if (!data.timezone.trim()) {
        errors.timezone = t('errorTimezone', 'Time zone is required');
    }
