/**
 * CompositeForm component - Interactive form for calculating composite charts.
 *
 * This component:
 * 1. Provides input fields for two subjects
 * 2. Submits to the composite chart API
 * 3. Displays the chart result (optionally)
 */

import { useState } from 'react';
import { cn, t } from '@/lib/utils';
import { fetchCompositeChart, fetchCompositeData } from '@/lib/api';
...
interface CompositeFormProps extends SubjectProps {
    className?: string;
    showChart?: boolean;
    [key: string]: unknown;
}
...
export function CompositeForm({
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        dispatchAstrologerEvent('astrologer:composite-data-updated', {
            firstSubject: s1,
            secondSubject: s2,
        });
        ...
        if (showChart) {
            try {
                const [chartRes, dataRes] = await Promise.all([
                    fetchCompositeChart(body).catch(() => null),
                    fetchCompositeData(body),
                ]);
    ...
    return (
        <Card className={cn('astrologer-composite-form', className)}>
            <CardHeader>
                <CardTitle>{t('compositeTitle', 'Composite Chart')}</CardTitle>
            </CardHeader>
            <CardContent>
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <Button type="submit" disabled={loading} className="w-full">
                        {loading ? (
                            ...
                            t('compositeSubmit', 'Calculate Composite Chart')
                        )}
                    </Button>
                </form>
