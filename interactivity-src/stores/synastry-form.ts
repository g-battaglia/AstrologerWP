/**
 * Interactivity store for the synastry-form block.
 *
 * Collects two subjects and submits to the synastry-chart REST endpoint.
 *
 * @package
 */
...
function getNonce(): string {
	const globals = (
		globalThis as unknown as {
			astrologerSynastryForm?: SynastryFormGlobal;
		}
	 ).astrologerSynastryForm;
...
const initialState: SynastryFormState = {
...
const { state } = store< {
	state: SynastryFormState;
	actions: SynastryFormActions;
} >( 'astrologer/synastry-form', {
	state: initialState,
	actions: {
		*updateField(
			event: Event
		...
		*submitForm(
			...
			try {
				const data = ( yield astrologerFetch< SynastryResponse >(
					'synastry-chart',
					{
