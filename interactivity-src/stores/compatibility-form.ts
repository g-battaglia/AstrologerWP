/**
 * Interactivity store for the compatibility-form block.
 *
 * Collects two subjects and submits to the relationship-score REST endpoint.
 *
 * @package
 */
...
interface CompatibilityFormActions {
	updateField: (
		event: Event
...
interface CompatibilityFormGlobal {
	restUrl?: string;
	nonce?: string;
}
...
function getNonce(): string {
	const globals = (
		globalThis as unknown as {
			astrologerCompatibilityForm?: CompatibilityFormGlobal;
		}
	 ).astrologerCompatibilityForm;
...
const { state } = store< {
	state: CompatibilityFormState;
	actions: CompatibilityFormActions;
} >( 'astrologer/compatibility-form', {
	state: initialState,
	actions: {
		*updateField(
			event: Event
		...
		*submitForm(
			...
			try {
				const data = ( yield astrologerFetch< CompatibilityResponse >(
					'relationship-score',
					{
				...
				state.summary = data.summary ?? data.description ?? '';
				state.hasResult = true;

				emit( 'astrologer:compatibility-calculated', {
					score: state.score,
